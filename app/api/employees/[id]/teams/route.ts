...
export const GET = createApiRoute({
  method: 'GET',
  path: '/api/employees/{id}/teams',
  summary: 'Get employee team assignments',
  description: 'Get all team assignments for an employee with optional filtering',
  tags: ['Employees'],
  security: 'adminAuth',
  request: {
  ...
  handler: async ({ params, query }) => {
    ...
    try {
      ...
      console.error("[api/employees/[id]/teams GET]", err)
      return { status: 500, data: formatError("Failed to fetch employee team assignments", "FETCH_FAILED") }
    }
  }
});
...
export const POST = createApiRoute({
  method: 'POST',
  path: '/api/employees/{id}/teams',
  summary: 'Assign team to employee',
  description: 'Assign employee to a team at a location',
  tags: ['Employees'],
  security: 'adminAuth',
  request: {
    params: employeeIdParamSchema,
  ...
  handler: async ({ params, body }) => {
    ...
    try {
      ...
      console.error("[api/employees/[id]/teams POST]", err)
      return { status: 500, data: formatError("Failed to assign team to employee", "ASSIGN_FAILED") }
    }
  }
});
