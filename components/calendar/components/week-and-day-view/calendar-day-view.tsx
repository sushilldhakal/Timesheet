...
export function CalendarDayView({ singleDayEvents, multiDayEvents }: IProps) {
  ...
  const { hours, earliestEventHour, latestEventHour } = getVisibleHours(visibleHours, singleDayEvents, false);
      ...
      <div className="hidden w-64 divide-y border-l md:block">
        ...
        <div className="flex-1 space-y-3">
          {currentEvents.length > 0 ? (
            ...
            <p className="p-4 text-center text-sm italic text-muted-foreground">No shifts scheduled for this day</p>
          )}
            ...
            <ScrollArea className="h-[422px] px-4" type="always">
              <div className="space-y-6 pb-4">
                {currentEvents.map(event => {
                  const user = event.user ? users.find(user => user.id === event.user.id) : null;
