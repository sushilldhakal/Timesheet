...
import { useCreateEmployeeTeam } from "@/lib/queries/employees"
...
interface EmployeeTeamAssignmentDialogProps {
  open: boolean
...
export function EmployeeTeamAssignmentDialog({
  open,
  onOpenChange,
  employeeId,
  employeeName,
  onSuccess,
}: EmployeeTeamAssignmentDialogProps) {
  ...
  const createTeamMutation = useCreateEmployeeTeam()
  ...
  // Reset team selection when location changes
      ...
      },
      {
        onSuccess: () => {
          ...
          setTimeout(() => {
            ...
            const selectedTeam = teams.find((t: { teamId: string }) => t.teamId === data.teamId)
            
            toast.success({
              title: "Team assigned",
              description: `${employeeName} has been assigned to ${(selectedTeam as { teamName?: string })?.teamName || "team"} at ${selectedLocation?.name || "location"}.`,
            })
          }, 1500)
        },
        onError: (error: any) => {
          ...
          if (result.code === "ROLE_NOT_ENABLED") {
            setValidationError(
              "This team is not enabled at the selected location. Please enable the team first."
            )
          } else if (result.code === "OVERLAPPING_ASSIGNMENT") {
            setValidationError(
              "This employee already has an overlapping assignment for this team at this location."
            )
  ...
  const handleClose = () => {
    if (!createTeamMutation.isPending && !showSuccess) {
      form.reset()
      setValidationError(null)
      onOpenChange(false)
    }
  ...
  const selectedTeam = teams.find((t: { teamId: string }) => t.teamId === selectedTeamId)
  ...
  return (
    <>
      ...
      submitLabel={createTeamMutation.isPending ? "Assigning..." : "Assign team"}
      loading={createTeamMutation.isPending}
      error={validationError}
      disabled={!selectedLocationId || !selectedTeamId}
      size="lg"
    >
      <Form {...form}>
                    ...
                    <SelectTrigger className="w-full">
                      <SelectValue placeholder={
                        ...
                        {field.value && selectedTeam && (
                          <div className="flex items-center gap-2">
                            {(selectedTeam as { teamColor?: string }).teamColor && (
                              <div
                                className="w-3 h-3 rounded-full shrink-0"
                                style={{ backgroundColor: (selectedTeam as { teamColor?: string }).teamColor }}
                              />
                            )}
                            <Briefcase className="h-4 w-4 text-muted-foreground" />
                            <span>{(selectedTeam as { teamName?: string }).teamName}</span>
                          </div>
                        )}
                      </SelectValue>
                    </SelectTrigger>
        ...
        <div className="flex flex-col items-center justify-center py-8 space-y-4">
          ...
          <div className="text-center space-y-2">
            <p className="text-sm text-muted-foreground">
              {employeeName} can now work as {(selectedTeam as { teamName?: string })?.teamName} at{" "}
