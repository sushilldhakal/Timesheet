...
interface StaffPanelProps {
  ...
  onPointerDragStart: (empId: string, categoryId: string, e: React.PointerEvent) => void
...
export function StaffPanel({
  ...
  const handlePointerDown =
    (emp: (typeof employees)[0]) => (e: React.PointerEvent<HTMLDivElement>): void => {
      e.currentTarget.setPointerCapture(e.pointerId)
      onPointerDragStart(emp.id, category.id, e)
    }
  ...
  return (
    ...
    >
      ...
      {unscheduled.map((emp) => (
        <div
          key={emp.id}
          onPointerDown={handlePointerDown(emp)}
          style={{
            display: "flex",
            alignItems: "center",
            gap: 8,
