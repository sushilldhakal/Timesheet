import { SHIFT_H, ROLE_HDR, clamp, snapToInterval } from '@/components/scheduling/core/constants-scheduler'

export interface GridConfig {
  visibleFrom:   number
  visibleTo:     number
  hourW:         number    // 96 * zoom
...
export function xToHour(x: number, dateIndex: number, cfg: GridConfig): number {
  if (cfg.isWeekView) {
    return clamp(cfg.visibleFrom + (x - dateIndex * cfg.colWWeek) / cfg.pxWeek, 0, 24)
  }
  return clamp(
