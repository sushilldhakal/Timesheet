...
export function detectConflicts(blocks: Block[]): ConflictMap {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    for (let j = i + 1; j < blocks.length; j++) {
      ...
      // Core Block model uses categoryId + ISO date + startH/endH
      if (
        blockA.categoryId === blockB.categoryId &&
        blockA.date === blockB.date &&
        blockA.startH < blockB.endH &&
        blockA.endH > blockB.startH
      ) {
...
export function findConflictingBlocks(
  ...
  for (const block of allBlocks) {
    ...
    // dateIndex is not part of the core Block model; callers should pre-filter by date if needed.
    void dateIndex
    if (
      block.categoryId === categoryId &&
      startHour < endHour && // Ensure valid time range
      block.startH < endHour &&
      block.endH > startHour
    ) {
