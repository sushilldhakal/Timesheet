  ...
  return (
    ...
    >
      {grouped.map(({ date, shifts: ds_ }) => {
        ...
        const handleShiftClick = (block: Block, resource: Resource): void => {
          if (!dragId) onShiftClick(block, resource)
        }

        const handleStatusToggle = (e: React.MouseEvent, shift: Block, draft: boolean): void => {
          e.stopPropagation()
          if (draft) onPublish(shift.id)
          else onUnpublish(shift.id)
        }
            ...
            {ds_.map((shift) => {
              ...
              return (
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={(e) => handleStatusToggle(e, shift, isDraft)}
                    className={cn(
