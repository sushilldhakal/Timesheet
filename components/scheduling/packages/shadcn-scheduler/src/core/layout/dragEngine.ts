...
export class DragEngine {
  ...
  startMove(
    ...
    // Compute where in the block the pointer landed so the cursor ghost
    // appears to be lifted from exactly the right spot
    let grabOffsetX = 0
    let grabOffsetY = 0
    let blockW = 120
    let blockH = 36
    ...
    this.active = {
      ...
      blockW, blockH,
      blockColor: "",   // set by GridView via _setBlockMeta
  ...
  /** Called by GridView after color is resolved — sets color on the active drag */
  setBlockMeta(color: string, label: string): void {
    if (this.active) {
      this.active.blockColor = color
      this.active.blockLabel = label
    }
  }
  ...
  private _updateGhost(clientX: number, clientY: number): void {
    ...
    // Notify React about hovered row (1 setState, batched)
    this.opts.onHoverCategory(newCat.id)

    // ── Update cursor ghost (raw pointer position — no snap) ──
    ...
    // ── Update snapped drop-zone ghost ──
  ...
  private _hideCursorGhost(): void {
    if (this.opts.cursorGhostEl) this.opts.cursorGhostEl.style.display = "none"
  }
}
