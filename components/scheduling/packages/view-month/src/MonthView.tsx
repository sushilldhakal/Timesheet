              ...
              >
                <div className="mb-0.5 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {copiedShift && (
                      <button
                        type="button"
                        onClick={() => { setShifts((prev) => [...prev, { ...copiedShift, id: nextUid(), date: toDateISO(d) }]); setCopiedShift?.(null) }}
                ...
                {dayShifts.slice(0, 3).map((shift) => {
                  ...
                  return (
                    <div
                      key={shift.id}
                      onPointerDown={(e) => onSPD(e, shift)}
                      onDoubleClick={(e) => { e.stopPropagation(); if (!dragId) onShiftClick(shift, category) }}
                      className={cn(
                ...
                {dayShifts.length > 3 && (
                  <div
                    onMouseEnter={(e) => setHoverMore({ date: d, rect: e.currentTarget.getBoundingClientRect() })}
                    onMouseLeave={() => setHoverMore(null)}
                    onClick={(e) => { e.stopPropagation(); setMoreShiftsDialog(d) }}
              ...
              {overflowShifts.map((s) => {
                ...
                return (
                  <div key={s.id} className="mb-0.5 flex justify-between gap-2 last:mb-0">
                    ...
                    <span className="text-muted-foreground">{fmt12(s.startH)}–{fmt12(s.endH)}</span>
                  </div>
                )
              })}
            </div>
