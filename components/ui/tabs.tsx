...
>(({ className, variant = "default", indicatorClassName, ...props }, ref) => {
  ...
  return (
    <div className="relative" ref={tabsListRef}>
      ...
      <div
        className={cn(
          "absolute z-0 rounded-md border border-transparent bg-primary shadow-sm transition-all duration-300 ease-in-out",
          indicatorClassName
        )}
...
function TabsTrigger({
  ...
  return (
    <TabsPrimitive.Trigger
      ...
      className={cn(
        "gap-1.5 rounded-lg border border-transparent px-3 py-2 text-sm font-medium group-data-vertical/tabs:py-[calc(--spacing(1.25))] [&_svg:not([class*='size-'])]:size-4 focus-visible:border-ring focus-visible:ring-ring/50 text-muted-foreground hover:text-foreground relative inline-flex h-full flex-1 items-center justify-center whitespace-nowrap transition-all group-data-vertical/tabs:w-full group-data-vertical/tabs:justify-start focus-visible:ring-[3px] focus-visible:outline-1 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0",
        "group-data-[variant=line]/tabs-list:bg-transparent group-data-[variant=line]/tabs-list:data-[state=active]:bg-transparent",
        "data-[state=active]:text-primary-foreground data-[state=active]:z-[1] data-[state=active]:relative",
